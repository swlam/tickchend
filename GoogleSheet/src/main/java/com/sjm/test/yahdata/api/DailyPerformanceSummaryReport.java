...
@Slf4j
public class DailyPerformanceSummaryReport {

    public static void printSimpleStatisticsResult(List<StockPerformanceDto> performanceResultList, String market, String ICONIC_CODE) {

        Optional<StockPerformanceDto> optional = performanceResultList.stream().filter(x->x.getCurrentStockBean().getStockCode().equalsIgnoreCase(ICONIC_CODE)).findFirst();
        //ICONIC_CODE
...
//		msgMain.append(ICONIC_CODE+" close\tPct.\tRSI(9)\tRSI(14)\t "+BASE_STOCK_B+" close\tPct.\tRSI(9)\tRSI(14)\t "+BASE_STOCK_C+" close\tPct.\tRSI(9)\tRSI(14)\t "+BASE_STOCK_D+" close\tPct.\tRSI(9)\tRSI(14)");//
    ...
    public static void analyseBySectorAndIndustry(List<StockPerformanceDto> perfromanceResultList) {
...
//		}
//        doAnaly("USD","allSymbols", perfromanceResultList, List.of(SectorAnalystHelper.CODE_ETF_SPY));
        doAnaly("HKD","allSymbols", perfromanceResultList, List.of(SectorAnalystHelper.CODE_INDEX_HSI));
        doAnaly("USD", "etf", perfromanceResultList, List.of(SectorAnalystHelper.CODE_ETF_SPY));
        doAnaly("USD","etf", perfromanceResultList, List.of(SectorAnalystHelper.CODE_ETF_QQQ));
        doAnaly("USD","etf", perfromanceResultList, List.of(SectorAnalystHelper.CODE_US_CHINA_CONCEPT));


        doAnaly("HKD","etf", perfromanceResultList, List.of(SectorAnalystHelper.CODE_INDEX_HSI));
        doAnaly("HKD","etf", perfromanceResultList, List.of(SectorAnalystHelper.CODE_ETF_HK_TECH));
        doAnaly("HKD","etf", perfromanceResultList, List.of(SectorAnalystHelper.CODE_ETF_HSCEI_ELEMENT));

//        doAnaly("CN","etf", perfromanceResultList, List.of(SectorAnalystHelper.LABEL_CN_CSI_300));
//        doAnaly("CN","etf", perfromanceResultList, List.of(SectorAnalystHelper.LABEL_CN_SZSE_ChiNext));

    }

    private static List<StockPerformanceDto> extract(String type, String matchingText, String transactionDate, List<StockPerformanceDto> perfromanceResultList){
        List<StockPerformanceDto> tmpList = null;
...
//			long upCntWithMoreVol = targetResultList.stream().filter( x-> x.getCurrentStockBean().getDayChgPct()>0.01 && x.getCurrentStockBean().getDayVolumeChgPct()>highVolRatio && x.getBelongETF().contains(filterName1)).count();
    ...
    private static void doAnaly(String mkt, String type, List<StockPerformanceDto> perfromanceResultList, List<String> sectorOrSymbolList) {

        double REQUIRED_TRADE_AMOUNT = (Const.IS_INTRADAY)?20.0:100.0;
        ...
        for(String sectorOrSymbol : sectorOrSymbolList) {

            Optional<StockPerformanceDto> optional = perfromanceResultList.stream().filter(x -> x.getCurrentStockBean().getStockCode().equalsIgnoreCase(sectorOrSymbol)).findFirst();
            StockPerformanceDto sectorOrSymbolBean;
            ...
            List<StockPerformanceDto> tmpList = extract(type, sectorOrSymbol, txnDate, perfromanceResultList);
            if(tmpList.isEmpty())
                continue;
            ...
            List<String> top10List = tmpList.stream()
                    .filter(x->x.getCurrentStockBean().getEstTradeAmount() > REQUIRED_TRADE_AMOUNT && sectorOrSymbolBean!=null && x.getCurrentStockBean().getDayChgPct()>sectorOrSymbolBean.getDailyChangePct())
...
//                                    +" "+ (stock.getSysPickLongCategory()+","+stock.getSysPickStagnantCategory()+","+stock.getSysPickShortCategory()).replaceAll("\\[|\\]", "").replaceAll(",,", ",")+" "
            ...
            List<String> top10O2HFor3DaysList = tmpList.stream()
                    .filter(x->x.getCurrentStockBean().getEstTradeAmount() > REQUIRED_TRADE_AMOUNT && x.getThreeDaysChangeO2HPct()>sectorOrSymbolBean.getDailyChangePct())
            ...
            List<String> top10O2LFor3DaysList = tmpList.stream()
                    .filter(x->x.getCurrentStockBean().getEstTradeAmount() > REQUIRED_TRADE_AMOUNT && x.getThreeDaysChangeO2LPct()<sectorOrSymbolBean.getDailyChangePct())
            ...
            List<String> top10DownList = tmpList.stream()
                    .filter(x->x.getCurrentStockBean().getEstTradeAmount() > REQUIRED_TRADE_AMOUNT && x.getCurrentStockBean().getDayChgPct()< sectorOrSymbolBean.getDailyChangePct())
                    .sorted((doc1, doc2) -> Double.compare(doc1.getDailyChangePct(), doc2.getDailyChangePct()))
                    .limit(LIMIT_RESULTS)
                    .map(
            ...
            List<String> top10WithGoodMAList = tmpList.stream()
                    .filter(x->x.getCurrentStockBean().getEstTradeAmount() > REQUIRED_TRADE_AMOUNT && x.getCurrentStockBean().getDayChgPct()>=0.01
...
//                    .filter(x->x.getCurrentStockBean().getEstTradeAmount() > REQUIRED_TRADE_AMOUNT && x.getStrongWeakTypeToday().getType().contains(PatternTrendHelper.STRONG))
//                    .sorted((doc1, doc2) -> Double.compare(doc2.getDailyChangePct(), doc1.getDailyChangePct()))
//                    .limit(LIMIT_RESULTS)
//                    .map(
//                            stock -> stock.getCurrentStockBean().getStockCode() + ": " + GeneralHelper.toPct(stock.getDailyChangePct())
...
//                    .filter(x->x.getCurrentStockBean().getEstTradeAmount() > REQUIRED_TRADE_AMOUNT && x.getStrongWeakTypeToday().getType().contains(PatternTrendHelper.WEAK))
//                    .sorted((doc1, doc2) -> Double.compare(doc1.getDailyChangePct(), doc2.getDailyChangePct()))
//                    .limit(LIMIT_RESULTS)
//                    .map(
//                            stock -> stock.getCurrentStockBean().getStockCode() + ": " + GeneralHelper.toPct(stock.getDailyChangePct())
...
//            long cnt2Weak = tmpList.stream().filter( x-> x.getStrongWeakTypeToday().getType().contains(PatternTrendHelper.WEAK) ).count();
            ...
            long cntUpBreakSign = tmpList.stream().filter( x-> x.getWaveShape()!=null && x.getWaveShape().getShapeResult().contains(Const.UP) && !x.getWaveShape().getShapeResult().contains(Const.D0) && !"Index".equalsIgnoreCase(x.getSector())).count();
            long cntDwBreakSign = tmpList.stream().filter( x-> x.getWaveShape()!=null && x.getWaveShape().getShapeResult().contains(Const.DOWN) && !x.getWaveShape().getShapeResult().contains(Const.D0) && !"Index".equalsIgnoreCase(x.getSector())).count();
            ...
            List<String> upBreakD0SignList = tmpList.stream()
                    .filter( x-> x.getWaveShape()!=null
                            ...
                            && x.getCurrentStockBean().getDayVolumeChgPct()>0.3
            ...
            String [] titles = new String[]
                    {"["+type+"]"
                            ...
                            , "Top"+top10List.size()+" (強於 "+sectorOrSymbol+")"
                            , "Top"+top10VolumnList.size()+" (Vol)"
                            , "Top"+top10O2HFor3DaysList.size()+" (強於 "+sectorOrSymbol+") 3天O2H"
                            , "Top"+top10WithGoodMAList.size()+" (UP) 小多頭"
                            , "Top"+top10DownList.size()+" (弱於 "+sectorOrSymbol+")"
                            , "Top"+top10O2LFor3DaysList.size()+" (弱於 "+sectorOrSymbol+")  3天O2L"
//                            , "Top"+top10TodayStrongList.size()+" (轉"+PatternTrendHelper.STRONG+")"
//                            , "Top"+top10TodayWeakList.size()+" (轉"+PatternTrendHelper.WEAK+")"
//                            , "強/弱/總數 "
                            , "Break (Up vs Down)"
                            , "Up Break (D0)"
                            , "Down Break (D0)"
...
//            sb.append("\n"); sb.append(sectorOrSymbol +"\t"+titles[14] +"\t" + cnt2Strong + " / "+ cnt2Weak + " / "+ cnt);
            sb.append("\n"); sb.append(sectorOrSymbol +"\t"+titles[12] +"\t" + "UpBreak % : "+GeneralHelper.toPct(cntUpBreakSignRatio) + "\t DwBreak %"+ GeneralHelper.toPct(cntDwBreakSignRatio) + "\t Up/Dw/總 : "+cntUpBreakSign + " / "+ cntDwBreakSign + " / "+ cnt);
            sb.append("\n"); sb.append(sectorOrSymbol +"\t"+titles[13] +"\t" + upBreakD0SignList.size() + ":\t "+ upBreakD0SignList);
            sb.append("\n"); sb.append(sectorOrSymbol +"\t"+titles[14] +"\t" + downBreakD0SignList.size() + ":\t "+ downBreakD0SignList);

            sb.append("\n"); sb.append(sectorOrSymbol +"\t"+titles[15] +"\t" + upBreakReadyList.size() + ":\t "+ upBreakReadyList);
            sb.append("\n"); sb.append(sectorOrSymbol +"\t"+titles[16] +"\t" + downBreakReadyList.size() + ":\t "+ downBreakReadyList);
