...
@Slf4j
public class PriceOverNNormalVolRule  extends VolRuleBase{
	private CandleEventTagEnum SIGN = CandleEventTagEnum.EVNT_PRICE_OVER_NORMAL_VOL;
	
	private static double VOL_RATE_RIGHT = 1.05; // example: prev-vol: 10, cur-vol: 12
	private static double VOL_RATE_LEFT = 0.95; // example: prev-vol: 10, cur-vol: 12
	...
	public boolean detect(List<StockBean> prevList, StockBean curr) {
		...
		try {
			if( curr.getVolume()> prev.getVolume() ) {
				if(curr.getVolume() / prev.getVolume() >=VOL_RATE_LEFT && curr.getVolume() / prev.getVolume()<= VOL_RATE_RIGHT) {
