...
public class PerformanceApi {
    ...
    private final MPatternAnaly mpatternAnaly = new MPatternAnaly();
//    private PriceVolumeRelationship pvr = new PriceVolumeRelationship();


    private final PriceTopBotWavePointHandler waveTopBotHandler = new PriceTopBotWavePointHandler();
    private final RulerDNTrendReversal downTrendReversal = new RulerDNTrendReversal();
//	private WaveRsiTopBotHandler waveRsiTopBotHandler = new WaveRsiTopBotHandler();

    private final ConcentratedVolumeAnalyzer stockAnalyzer = new ConcentratedVolumeAnalyzer();

    private final BollingerBandApi bollingerBandApi = new BollingerBandApi();
    public List<StockPerformanceDto> doBenchmarks(List<String> stockPool, List<StockBean> fullTrunkList,
...
//        );
        ...
        List<StockPerformanceDto> recentHighList = new ArrayList<StockPerformanceDto>(10);
...
//				MovingAvgCrossResultBean ma50200CrossDownDate = maStatusengine.findRecentMACrossDownDate(stockList, 50, 200);
                ...
                StockPerformanceDto recentHBean = this.findRecordHigh(stockList);
...
//		log.info("Skipped Ticker: "+skippedTickers.size());
    ...
    public StockPerformanceDto findRecordHigh(List<StockBean> stockList) {
        ...
        StockPerformanceDto rtn = new StockPerformanceDto(stockList);
...
//		public String getBreakBBUpBanInPassFewDays(List<StockBean> srcstockList, int days) {
//			if(srcstockList.size() < days) {
//				return Const.SPACE;
//			}
//
...
//					x-> x.getC()>x.getBollingerBand().getUpper() && x.isRiseToday()
//					).findFirst();
//
//		}
